...
static inline int32_t getInt32LE(const std::vector<uint8_t> &data, int index) {
  return (int32_t)((data[index]) | (data[index + 1] << 8) |
...
static inline uint16_t getUint16BE(const std::vector<uint8_t> &data,
                                   int index) {
...
static bool decodeRuuvi(const std::vector<uint8_t> &data, JsonDocument &json) {
  ...
  if (accX != (int16_t)0x8000)
    json["accx"] = accX;
  if (accY != (int16_t)0x8000)
    json["accy"] = accY;
  if (accZ != (int16_t)0x8000)
    json["accz"] = accZ;
...
static bool decodeMopeka(const std::vector<uint8_t> &data, JsonDocument &json) {
  ...
  json["lvl_prop"] = round1(
      raw_level *
      (MOPEKA_TANK_LEVEL_COEFFICIENTS_PROPANE_0 +
       (MOPEKA_TANK_LEVEL_COEFFICIENTS_PROPANE_1 * raw_temp) +
       (MOPEKA_TANK_LEVEL_COEFFICIENTS_PROPANE_2 * raw_temp * raw_temp)));
  return true;
}

static bool decodeTPMS100(const std::vector<uint8_t> &data,
                          JsonDocument &json) {
  if (data.size() != 18)
    return false;
...
static bool decodeTPMS00AC(const std::vector<uint8_t> &data,
                           JsonDocument &json) {
  if (data.size() != 15)
    return false;
...
static bool decodeOtodata(const std::vector<uint8_t> &data,
                          JsonDocument &json) {
  switch (data.size()) {
  case 21:
    json["dev"] = "Otodata";
...
static bool decodeRotarexELG(const std::vector<uint8_t> &data,
                             JsonDocument &json, JsonObject BLEdata) {
  if (data.size() != 12)
    return false;
...
static bool decodeBTHome(JsonObject BLEdata, JsonDocument &json,
  ...
  BTHomeDecodeResult bthRes = decoder.parseBTHomeV2(
      std::string(sd.begin(), sd.end()), BLEdata["mac"], key);
...
class ScanCallback : public BLEAdvertisedDeviceCallbacks {
  void onResult(BLEAdvertisedDevice advertisedDevice) override {
    ...
    if (advertisedDevice.haveServiceUUID())
      BLEdata["svcuuid"] =
          (char *)advertisedDevice.getServiceUUID().toString().c_str();

    int sdCount = advertisedDevice.getServiceDataUUIDCount();
    if (sdCount > 0) {
      int idx = sdCount - 1;
      BLEdata["svduuid"] =
          (char *)advertisedDevice.getServiceDataUUID(idx).toString().c_str();
      String hexData;
...
bool BLEScanner::deliver(JsonDocument &rawDoc, JsonDocument &outDoc) {
  ...
  if (rawDoc["svduuid"].is<const char *>() &&
    ...
    decoded = decodeBTHome(rawDoc.as<JsonObject>(), outDoc, _impl->bthDecoder,
                           _impl->bthKey);
    ...
    if (hexStringToVector(rawDoc["mfd"], mfd) && mfd.size() >= 2) {
      ...
      case 0x03B1:
        ...
        break;
      case 0xffff:
        decoded = decodeRotarexELG(mfd, outDoc, rawDoc.as<JsonObject>());
        break;
      case 0x094f:
