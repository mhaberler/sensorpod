...
void wifi_setup() {
  ...
  wifiMulti.setStrictMode(
      false); // Default is true.  Library will disconnect and forget currently
              // connected AP if it's not in the AP list.
...
void wifi_loop() {
  ...
  if (ws ^ wifi_status) {
    ...
    switch (ws) {
    case WL_CONNECTED: {
      log_w("WiFi: Connected to %s %s RSSI %d  IP: %s", WiFi.STA.SSID().c_str(),
